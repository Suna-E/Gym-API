...
export const CreateSession = async (req: Request, res: Response) => {
  try {
    const { className, trainer, capacity, timeSlot} = req.body;

    if (!className || !trainer || !capacity || !timeSlot.startTime || !timeSlot.endTime) { 
    ...
    ////////////under maintenance
    /*
    const overlappingSession = await ClassSession.findOne({
    instructor, // Optional: ensure the specific instructor isn't double-booked
    'timeSlot.startTime': { $lt: newEnd },   // Existing starts before new ends
    'timeSlot.endTime': { $gt: newStart }    // Existing ends after new starts
  });
...
*/
      res.status(201).json({ message: ' CONGRATULATIONS!!!  Your  Class session is succesfully created '});
    } 
      catch (error: any) {
...
export const UpdateSession = async (req: Request, res: Response) => {
  try {
    const { id} = req.params;
    const { className, trainer, startTime, endTime, capacity } = req.body;
